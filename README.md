...
Windows 双击 `启动网页版.bat`；Linux/macOS/Git Bash 使用 `bash 启动网页版.sh`。首次启动会自动安装依赖并构建，随后浏览器打开：
...
- “AI 识别当前遭遇”使用战术终端中单独选择的模型连接与 AIRP 预设；也可用内置文本编辑器手工建立 `EncounterDraft`。识别前可在“额外需求”输入框写明更大的战场规模、夜战或需强调的战场设定，随识别请求一并交给战斗 AI。
- 纯手操逐单位停顿；半自动先预览策略及任一接管条件；全自动在本地推进到失能、安全暂停、200 回合上限或 10 回合僵局。
- 所有能力脚本在 QuickJS/WASM 能力沙箱内运行，首次使用必须完成 100 组固定种子审查并显式批准。
- 第 2 步数字化建模：模型服务超时或出错会自动退避重试且不占用修复名额；五次修复失败后可用“继续重试”携带完整修复历史再试，或转人工修正。
- 结果结算阶段的“战后处置”输入框可写明搜刮战利品、安葬死者等要求，随融合战报一并交给正文 AI；写入剧情后自动清空。
...
- “世界书管理”面板以原卡 3.2.6 世界书 V3 格式管理预设之外的自定义条目（关键词触发、常驻、聊天深度注入、EJS、导入导出），与角色卡内置世界书并行生效，并预置战斗交接与战后处置规则条目。
